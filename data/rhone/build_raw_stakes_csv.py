#!/usr/bin/env python3
"""
Build data/rhone/SMB_rhone_<START>-<END>_rawstakes_utm32N.csv from the
GLAMOS Swiss Glacier Point Mass Balance archive (raw stake / snow-pit
readings, NOT the interpolated elevation curve).

Mirrors data/aletsch/build_raw_stakes_csv.py; see that file for the source
DOI and pipeline rationale.

Output columns:
  X, Y, Alt, Annual_SMB (m w.e.)

NB: smb_inference's plotting overlay reads only Alt + Annual_SMB, so the
CRS of X/Y is documentary — kept as UTM 32N for parity with the Aletsch
file.
"""
...
DAT_FILE = os.path.join(HERE, "raw_glamos", "rhone_annual.dat")

START_YEAR = 2000
END_YEAR = 2020

OUT_CSV = os.path.join(HERE, f"SMB_rhone_{START_YEAR}-{END_YEAR}_rawstakes_utm32N.csv")
