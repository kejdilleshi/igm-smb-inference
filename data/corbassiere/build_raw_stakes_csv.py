#!/usr/bin/env python3
"""
Build data/corbassiere/SMB_corbassiere_<START>-<END>_rawstakes_utm32N.csv from the
GLAMOS Swiss Glacier Point Mass Balance archive (raw stake / snow-pit
readings, NOT the interpolated elevation curve).

Mirrors data/rhone/build_raw_stakes_csv.py; see that file for the source
DOI and pipeline rationale. Glacier de Corbassiere is RGI60-11.02766 in the
OGGM/COP-DEM pipeline (data/oggm/*.py).

Output columns:
  X, Y, Alt, Annual_SMB (m w.e.), Year (hydrological year start)

NB: smb_inference's plotting overlay reads only Alt + Annual_SMB + Year, so
the CRS of X/Y is documentary — kept as UTM 32N for parity with the other
glaciers.
"""
...
DAT_FILE = os.path.join(HERE, "raw_glamos", "corbassiere_annual.dat")
...
def main():
    ...
    out_csv = os.path.join(
        HERE, f"SMB_corbassiere_{start_year}-{end_year}_rawstakes_utm32N.csv")
