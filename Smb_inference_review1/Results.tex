...
We evaluate the method on three Alpine glaciers spanning a wide range of size and quality
...
%GJ: ot sure I like your notations ... also what sense to name a math symbol $c$ since it does not relate to any equation of your paper (unlike $H$).
$H_\mathrm{DA}$ and a basal-sliding field $c_\mathrm{DA}$ that reproduce both the
observed surface velocity and the GPR thickness transects, 
%GJ: I can not see any convergence result in Fig 2, may be good to add such a convegrence graph
converging within
...
%GJ: no idea what you mean here, rephrase please
frozen dynamical substrate

for the transient forward run (Sect.~\ref{sec:inference}).
%GJ: If you say this, you should supply result, and evidence the add-value with and without DA, can you do this ? I would be very interested to see it !
...
\begin{figure}
    ...
    \label{fig:da}
\end{figure}
%GJ: it would be more interesting I think to show i) obs speed ii) modeled speed iii) the diff between the 2 as you have now. The slidinco is super difficult to interpret for a number of people, what do you want to show displaying this hard-to-catch quantity ?
% ------------------------------------------------------------------------
...
optimiser converges in 50 function evaluations.
...
%GJ: typically this would be good to annonce these validation data in a data section before, and also have the validation data appearing in the big picture figure
...
%GJ: can you not already lead the comparision, assess the added-value of the transiency on argentiere ?
...
\label{sec:transfer}% GJ: the fact that you do no retune is impressive and strong results, but I can not not think this may be more coincidental than anything else. In fact the main point here is that you show that the speed data assimilation is robust, which is great news, but I still expect this may behave differently if yu were doing this on like 100 glaciers, not only 3 lare size ones.
...
velocity-calibrated dynamical state is key to recover the elevation
...
% ------------------------------------------------------------
\subsection{Sensitivity to the basal sliding coefficient}\label{sec:sensitivity} %GJ : if the main message, slidinc param is rather unsensitive, it should not take too much space in the paper, you may shorten it a bit. 
